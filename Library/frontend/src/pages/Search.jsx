...
import {
    ...
    Book as BookIcon, Clock, ArrowRight, Loader2
} from 'lucide-react';
...
const Search = () => {
    ...
    console.log('📚 Search.jsx Pagination Debug:', {
    ...
    const handleBorrow = async (book) => {
        if (!book.isbn) {
            return setMessage({ text: 'Cannot borrow book without ISBN', type: 'error' });
        }
        ...
        try {
            // Call the borrow endpoint with all book details
            // Backend will auto-import if book doesn't exist
            ...
            setMessage({ text: res.data.message || `Successfully borrowed "${book.title}"`, type: 'success' });

            // Update local state to show book is now in library and borrowed
            ...
            <section style={{ textAlign: 'center', padding: '5rem 0' }} className="hero-section">
                ...
                <form onSubmit={handleSearchSubmit} style={{ maxWidth: '700px', margin: '0 auto', position: 'relative' }}>
                    ...
                    <button type="submit" className="btn-primary" style={{ position: 'absolute', right: '0.6rem', top: '0.6rem', bottom: '0.6rem', borderRadius: '40px', padding: '0 2rem', fontSize: '1.1rem' }} disabled={loading}>
                        {loading ? <Loader2 className="spin" /> : 'Explore'}
                    </button>
                </form>
            </section>
                                    ...
                                    <div style={{ height: '280px', display: 'flex', justifyContent: 'center', marginBottom: '1.5rem', background: 'rgba(0,0,0,0.05)', borderRadius: '8px', padding: '10px' }}>
                                        {book.cover_url ? (
                                            <img
                                                ...
                                                onError={(e) => { e.target.src = "https://placehold.co/150x220/334155/f8fafc?text=No+Cover"; }}
                                            />
                                        ) : (
                                        ...
                                        {book.in_library ? (
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                ...
                                                {book.available && (
                                                    ...
                                                    >
                                                        {borrowing === book.isbn ? <Loader2 size={16} className="spin" /> : 'Borrow'}
                                                    </button>
                                                )}
                                            </div>
                                        ) : (
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                <span style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                                                    <BookOpen size={14} /> Not in Library
                                                </span>
                                                <button
                                                    className="btn-primary"
                                                    style={{ padding: '6px 16px', fontSize: '0.85rem' }}
                                                    ...
                                                    onClick={() => handleBorrow(book)}
                                                >
                                                    {borrowing === book.isbn ? 'Adding...' : 'Add & Borrow'}
                                                </button>
                                            </div>
                                        )}
                                    </div>
